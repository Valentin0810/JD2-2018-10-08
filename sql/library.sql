...
INSERT INTO my_library.author (full_name, date_birth, date_death, brief_information) VALUES
  ...
  ('Оуэн Кинг', '21 февраля 1977','наст время','Оуэн Филипп Кинг - американский автор и младший сын авторов Стивена и Табиты Кинг.');
...
INSERT INTO my_library.book (genre_id, author_id, name, publishing_house_id, year_publishing, pages) VALUES
  ((SELECT id
...
INSERT INTO my_library.reader (full_name, address, phone_number, role, login, password, passport_series) VALUES
  ...
  ('Малахов Андрей Константинович', 'ул. Краснознаменная 31/2', 532112525, 'READER', 'Malahov@gmail.com', '1111',
   'AA1212121');
...
INSERT INTO my_library.book_author (book_id, author_id) VALUES
  ((SELECT id
    FROM my_library.book
    WHERE name = 'Оно'), (SELECT id
                          FROM my_library.author
                          WHERE full_name = 'Стивен Кинг')),
  ((SELECT id
    FROM my_library.book
    WHERE name = 'Бездна голодных глаз'), (SELECT id
                                           FROM my_library.author
                                           WHERE full_name = 'Генри Лайон Олди')),
  ((SELECT id
    FROM my_library.book
    WHERE name = 'Зов предков'), (SELECT id
                                  FROM my_library.author
                                  WHERE full_name = 'Джек Лондон'));
